...
(defn nodes-for-wire
  ...
  (->>
   ...
   parse
   (map instruction)
   (mapcat expand-to-steps)
   (reductions next-step [0 0])))

(defn manhattan-dist
  "calculate manhattan distance from origin point to destination point on the grid"
  [[ox oy] [x y]]
...
(defn total-wire-dist
  ...
  [nodes-1 nodes-2 dest]
  (let [dist-1 (wire-dist nodes-1 dest)
